...
public:
    ...
    void backward(CuMatrix Label, CuMatrix Pred)
    {
        ...
        cudaDeviceSynchronize();
        d_a = d_a.dot(z_dst);
        d_a = cu.reduceColumns(d_a);

        auto AdP = cu.aggregate(e, d_P);
        CuMatrix coef_vec;
        coef_vec.devPtr = d_A.devPtr;
        auto dAZ = cu.scaleRowsByVector(z_src, coef_vec);
        auto dz = cu.hadamardAdd(dAZ, AdP);
        auto dw=h.dot(dz,true,false);
...
int main()
{
    ...
    gat.backward(CuMatrix(),CuMatrix());
    return 0;
}
...
//Test Sum Row
// float feat_arr[3 * 7];
// for (int i = 0; i < 35; ++i) feat_arr[i] = 0.01 * i;
// CuMatrix features = cu.wrapMatrix(Matrix(7, 3, feat_arr));
// cout<<features.str();
